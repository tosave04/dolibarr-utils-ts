 ...
 * @param	{boolean}		[onlyId=false]	If true, returns only the proposal ID; otherwise, returns the full proposal object.
 * @param	{DolibarrApi}	[api=getApi()]	An instance of the Dolibarr API.
 * @returns	{Promise<{ proposalId: number; proposal?: Proposal }>}	The proposal ID or the full proposal object.
...
): Promise<{ proposalId: number; proposal?: Proposal }> => {
	try {
