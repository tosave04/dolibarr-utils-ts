...
describe("forgeSQLFromFilters", () => {
	it("should generate custom filter string for basic equality", () => {
		const filters: SQLFilterObject[] = [{ field: "t.status", operator: "=", value: 1 }]
	...
	it("should generate custom filter string for LIKE operator", () => {
		const filters: SQLFilterObject[] = [{ field: "t.ref", operator: "like", value: "SO-%" }]
		const result = forgeSQLFilters(filters)
		expect(result).toBe("(t.ref:like:'SO-%')")
	})

	it("should generate custom filter string for IN operator", () => {
		const filters: SQLFilterObject[] = [{ field: "t.status", operator: "in", value: [1, 2, 3] }]
		const result = forgeSQLFilters(filters)
		expect(result).toBe("(t.status:in:('1','2','3'))")
	})

	it("should generate custom filter string for NULL check", () => {
		const filters: SQLFilterObject[] = [{ field: "t.nature", operator: "is", value: null }]
		const result = forgeSQLFilters(filters)
	...
	it("should generate custom filter string for multiple filters", () => {
		const filters: SQLFilterObject[] = [
			{ field: "t.ref", operator: "like", value: "SO-%" },
			{ field: "t.date_creation", operator: "<", value: "20160101" },
			{ field: "t.status", operator: "in", value: [1, 2, 3] },
		]
		const result = forgeSQLFilters(filters)
	...
	it("should escape single quotes in string values", () => {
		const filters: SQLFilterObject[] = [{ field: "t.comment", operator: "like", value: "O'Reilly" }]
		const result = forgeSQLFilters(filters)
